...
from .features import process_features, CadenceScaler
...
class BDPArtifacts:
    ...
    scaler:        Optional[CadenceScaler]            = None   # CadenceScaler (wraps StandardScaler + EDA state)
...
class BDPPipeline:
    ...
    Stages
    ------
    1.  load         — Ingest from CSV/Parquet/unified; engineer all features.
    2.  scale        — Pair-level aggregation, EDA validation, StandardScaler.
    3.  prefilter    — Domain-knowledge pre-filter (infra, fanin, conn state).
    4.  filter       — Isolation Forest anomaly pre-filter + SHAP.
    5.  sax_screen   — SAX symbolic pre-filter per channel.
    6.  periodicity  — Per-pair ACF + FFT beacon scoring.
    7.  score        — Heuristic priority scoring per pair.
    8.  changepoints — PELT beacon start / interval shift detection.
    9.  corroborate  — DNS + HTTP + TLS cross-source validation.
    10. mitre_map    — ATT&CK technique annotation on confirmed leads.
    """
    ...
    def scale(
        ...
        """
        Aggregate flows to channel-level pairs, run EDA validation,
        and Z-score standardise IForest features.

        EDA steps (wired to ScalingConfig)
        ------------------------------------
        - Median imputation for NaN values (not fillna(0))
        - Skewness-triggered log1p for non-protected features
        ...
        df_scaled, cadence_scaler, _, _ = process_features(df, self.cfg)
        return df_scaled, cadence_scaler
    ...
    def filter(
        ...
        visualize: bool = True,
    ) -> tuple[pd.DataFrame, pd.DataFrame, IsolationForest, float, pd.DataFrame]:
        """Isolation Forest anomaly pre-filter + SHAP explanation.

        Returns
        -------
        df_annotated, anomalies_df, model, stability, shap_df

        shap_df contains one SHAP-value column per IForest feature for every
        pair in df_scaled. Anomalous pairs are joined with shap_df in art so
        the analyst can see exactly which behavioral signals drove each flag.
        """
        ...
        # SHAP — run on full pair set so beeswarm covers normal + anomalous
        shap_df = explain_with_shap(model, df_annotated)
        if visualize and not shap_df.empty:
            plot_shap_beeswarm(model, df_annotated)

        return df_annotated, anomalies_df, model, stability, shap_df
    ...
    def corroborate(
        ...
        dns_df:          Optional[pd.DataFrame] = None,
        http_df:         Optional[pd.DataFrame] = None,
        ssl_df:          Optional[pd.DataFrame] = None,
        visualize:       bool = True,
    ) -> pd.DataFrame:
        """Cross-validate beacon candidates against DNS, HTTP, and TLS evidence.

        Accepts either file paths (dns_log_path / http_log_path / ssl_log_path)
        OR pre-loaded DataFrames (dns_df / http_df / ssl_df).  When the unified
        ingestion mode is active, the pipeline passes DataFrames directly.
        """
        log.info("Stage 9: corroborate()")

        has_paths = any(p is not None for p in (dns_log_path, http_log_path, ssl_log_path))
        has_dfs   = any(d is not None for d in (dns_df, http_df, ssl_df))

        if not has_paths and not has_dfs:
            log.warning("No DNS, HTTP, or SSL logs provided — skipping corroboration.")
            return pd.DataFrame()

        # Path-based loading (original behaviour)
        if dns_log_path and dns_df is None:
            dns_df  = load_dns_logs(dns_log_path, self.cfg)
        if http_log_path and http_df is None:
            http_df = load_http_logs(http_log_path, self.cfg)
        if ssl_log_path and ssl_df is None:
            ssl_df  = load_ssl_logs(ssl_log_path, self.cfg)

        # Fallback to empty frames where needed
        if dns_df is None:
            dns_df = pd.DataFrame()
        if http_df is None:
    ...
    def run(
        ...
        art.scaled, art.anomalies, \
            art.iforest_model, art.iforest_stability, \
            art.shap_values                              = self.filter(art.scaled, visualize)
        ...
        # In unified mode the slices are already loaded as DataFrames on cfg.
        _unified = getattr(self.cfg, "_unified_slices", None)
        if _unified is not None:
            from .corroboration import load_dns_from_df, load_http_from_df, load_ssl_from_df
            _dns_df  = load_dns_from_df(_unified["dns"], self.cfg)  if not _unified["dns"].empty  else None
            _http_df = load_http_from_df(_unified["http"], self.cfg) if not _unified["http"].empty else None
            _ssl_df  = load_ssl_from_df(_unified["ssl"], self.cfg)  if not _unified["ssl"].empty  else None

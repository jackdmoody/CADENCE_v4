...
def load_csv(path: Path) -> pd.DataFrame:
    """Load a previously saved Zeek conn CSV."""
    log.info("Loading CSV: %s", path)
    return pd.read_csv(path, low_memory=False)


def load_parquet(path: Path) -> pd.DataFrame:
    """Load a Zeek conn log from a Parquet file."""
    log.info("Loading Parquet: %s", path)
    return pd.read_parquet(path)


# ---------------------------------------------------------------------------
# Unified single-file ingestion  (log_type column)
# ---------------------------------------------------------------------------

_UNIFIED_LOG_TYPES = ("conn", "dns", "http", "ssl")


def load_unified(path: Path) -> dict:
    """
    Load a single CSV or Parquet file that contains all Zeek log types in one
    table, distinguished by a log_type column with values:
        conn, dns, http, ssl

    Returns
    -------
    dict with keys conn, dns, http, ssl, each being a DataFrame
    (empty if that log type was not present in the file).
    """
    import pathlib
    path = pathlib.Path(path)
    log.info("Loading unified log file: %s", path)
    ...
    if "log_type" not in df.columns:
        raise ValueError(
            "Unified input file must contain a log_type column "
            f"(values: {_UNIFIED_LOG_TYPES}). Found columns: {list(df.columns)[:30]}"
        )

    result = {}
    for lt in _UNIFIED_LOG_TYPES:
        slice_df = df[df["log_type"] == lt].drop(columns=["log_type"]).copy().reset_index(drop=True)
        result[lt] = slice_df
        log.info("  unified[%s]: %d rows", lt, len(slice_df))

    missing = [lt for lt in _UNIFIED_LOG_TYPES if result[lt].empty]
    if missing:
        log.warning("Unified file is missing log types: %s", missing)
...
def load_and_prepare(cfg: BDPConfig) -> pd.DataFrame:
    ...
    Loads data from ISF, CSV, Parquet, or unified single-file, applies all
    transformations, and returns a clean DataFrame ready for the scaling stage.

    Input priority (first non-None wins):
        cfg.io.input_unified  — single file with log_type column; conn slice used here.
        cfg.io.input_parquet  — Parquet conn log.
        cfg.io.input_csv      — CSV conn log (original behaviour).
        (fallback)            — ISF query.
    ...
    """
    # --- 1. Ingest ---
    if cfg.io.input_unified is not None:
        slices = load_unified(cfg.io.input_unified)
        df = slices["conn"]
        if df.empty:
            raise ValueError("Unified file contains no rows with log_type==conn")
        # Store the other slices on cfg so pipeline.run() can use them
        # without requiring separate path args.
        cfg._unified_slices = slices
    elif cfg.io.input_parquet is not None:
        df = load_parquet(cfg.io.input_parquet)
    elif cfg.io.input_csv is not None:

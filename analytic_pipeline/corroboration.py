...
def load_ssl_logs(path: str, cfg: BDPConfig) -> pd.DataFrame:
    """Load and normalise a Zeek ssl.log CSV export.
...
# ---------------------------------------------------------------------------
# DataFrame-based loaders (unified / in-memory ingestion)
# ---------------------------------------------------------------------------

def _apply_time_filter(df: pd.DataFrame, cfg: BDPConfig, ts_col: str = "ts") -> pd.DataFrame:
    """Apply query_start / query_end filter to a DataFrame with a parsed timestamp column."""
    if cfg.io.query_start:
        df = df[df[ts_col] >= pd.to_datetime(cfg.io.query_start, utc=True)]
    if cfg.io.query_end:
        df = df[df[ts_col] <= pd.to_datetime(cfg.io.query_end, utc=True)]
    return df


def load_dns_from_df(df: pd.DataFrame, cfg: BDPConfig) -> pd.DataFrame:
    """
    Normalise a DNS DataFrame (e.g. the dns slice from load_unified()) into the
    same schema produced by load_dns_logs().  Renames ISF field names where
    needed, parses timestamps, and applies the configured time window filter.
    """
    df = df.copy()
    rename = {
        "ts": "ts", "id.orig_h": "src_ip", "query": "query",
    ...
    for col in ("ts", "src_ip", "query"):
        if col not in df.columns:
            raise ValueError(f"DNS DataFrame missing required column {col!r}. Available: {list(df.columns)[:20]}")

    df["ts"] = pd.to_datetime(df["ts"], unit="s", utc=True, errors="coerce")
    df = _apply_time_filter(df, cfg, "ts")
    df = df.dropna(subset=["ts", "src_ip", "query"]).copy()
    log.info("DNS DataFrame loaded: %d records, %d unique src_ips", len(df), df["src_ip"].nunique())
    return df


def load_http_from_df(df: pd.DataFrame, cfg: BDPConfig) -> pd.DataFrame:
    """
    Normalise an HTTP DataFrame (e.g. the http slice from load_unified()) into
    the same schema produced by load_http_logs().
    """
    df = df.copy()
    rename = {
        "id.orig_h": "src_ip", "id.resp_h": "dst_ip",
    ...
    df = df.dropna(subset=["src_ip"]).copy()
    log.info("HTTP DataFrame loaded: %d records, %d unique src_ips", len(df), df["src_ip"].nunique())
    return df


def load_ssl_from_df(df: pd.DataFrame, cfg: BDPConfig) -> pd.DataFrame:
    """
    Normalise an SSL DataFrame (e.g. the ssl slice from load_unified()) into
    the same schema produced by load_ssl_logs().
    """
    df = df.copy()
    rename = {
        "ts": "ts", "id.orig_h": "src_ip", "id.resp_h": "dst_ip",
        "server_name": "server_name", "ja3": "ja3", "ja3s": "ja3s",
        "cert_chain_fuids": "cert_chain_fuids",
        "validation_status": "validation_status",
        "resumed": "resumed", "established": "established",
        "cert_chain": "cert_chain_fuids",
    }
    for src, dst in rename.items():
        if src in df.columns and dst not in df.columns:
            df = df.rename(columns={src: dst})
    ...
    if "ts" not in df.columns:
        raise ValueError(f"SSL DataFrame missing ts column. Available: {list(df.columns)[:20]}")

    df["ts"] = pd.to_datetime(df["ts"], unit="s", utc=True, errors="coerce")
    df = _apply_time_filter(df, cfg, "ts")
    df = df.dropna(subset=["ts"]).copy()
    log.info("SSL DataFrame loaded: %d records", len(df))
    return df

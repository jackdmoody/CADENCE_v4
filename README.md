...
The easiest way to run CADENCE. Exposes all configuration parameters and provides
live log streaming, pipeline funnel metrics, SHAP visualizations, and download buttons
for all outputs.
...
| **Separate files** | Individual CSV or Parquet files per log type — the original mode. |
| **ISF query** | SQL-based ingestion via `ionic_scripting_framework` (set connection params in I/O tab). |
